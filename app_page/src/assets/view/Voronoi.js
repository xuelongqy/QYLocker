// Voronoi Graffiti - mouse over for fun colorful shapes!
// See http://blog.neave.com/post/65800598584/digital-graffiti
// by Paul Neave
// @neave
...
(function ($) {
    // 添加jquery扩展
    $.fn.voronoi = function (isAnimation, colorRange) {
        // 默认颜色方案
        var colorRangeDefault = {
            cr: "0..255",
            cg: "0..255",
            cb: "0..255"
        }
        ...
        if (arguments.length > 1) {
            try {
                mColorRange = parsingColorRange(colorRange)
            }catch (e) {
                console.log(e)
                mColorRange = parsingColorRange(colorRangeDefault)
            }
        }else {
            mColorRange = parsingColorRange(colorRangeDefault)
        }
        ...
        // 动画
        if (mIsAnimation) {
            var Voronoi = VoronoiFactory(mColorRange)
            var canvas = document.createElement('canvas');
            this.prepend(canvas);
            init = () =>{
                Voronoi.stop();
                Voronoi.init(canvas, this.width(), this.height());
                Voronoi.start();
            };
            Voronoi.initMouse();
            // 设置元素大小改变监听
 ...
 * @throws
 */
function parsingColorRange(colorRange) {
    // 定义颜色异常
    var colorException = new Error('Color format error!');
    // 判断颜色格式
    if (colorRange instanceof Array) { // 数组直接返回
        return colorRange
    }
    if (colorRange instanceof Object) { // 如果是对象则解析
        // 颜色容器
        var colorContainer = {
            rmin: 0,
            rmax: 0,
        ...
        // 红
        var cr = splitC(colorRange.cr)
        colorContainer.rmin = cr[0]
        colorContainer.rmax = cr[1]
        // 绿
        var cg = splitC(colorRange.cg)
        colorContainer.gmin = cg[0]
        colorContainer.gmax = cg[1]
        // 蓝
        var cb = splitC(colorRange.cb)
        colorContainer.bmin = cb[0]
        colorContainer.bmax = cb[1]
        return colorContainer

        // 切割颜色范围
        function splitC(c) {
            var cs = c.split('..')
            if (cs[0] > 225 || cs[1] > 255) {
 ...
 * @param colorRange 颜色范围
*/
function VoronoiFactory(colorContainer) {
    console.log(colorContainer)
    // 颜色方案
    var mColorContainer = colorContainer
    ...
    function getColor(cell) {
        ...
        // 判断颜色范围
        if (mColorContainer instanceof Array) {
            var index = cr % mColorContainer.length
            return mColorContainer[index]
        }else if (mColorContainer instanceof Object) {
            cr = rangeValue(cr, mColorContainer.rmin, mColorContainer.rmax)
            cg = rangeValue(cg, mColorContainer.gmin, mColorContainer.gmax)
            cb = rangeValue(cb, mColorContainer.bmin, mColorContainer.bmax)
        }
    ...
    function loop() {
        ...
        while (i--) {
            ...
            if (d < followDist) {
                ...
                vx -= cos_angle * a + Math.random() * 2 - 1;
                vy -= sin_angle * a + Math.random() * 2 - 1;
            }
